#!/bin/bash
#
# Format repository source files according to the project's Google-style setup.
...
if [[ ! -x "${ROOT_DIR}/tools/google-style/bin/shfmt" ]]; then
  echo "missing formatter: tools/google-style/bin/shfmt" >&2
  exit 1
fi

if [[ ! -x "${ROOT_DIR}/tools/google-style/bin/ktlint" ]]; then
  echo "missing formatter: tools/google-style/bin/ktlint" >&2
  exit 1
fi
...
"${ROOT_DIR}/tools/google-style/bin/shfmt" -w -i 2 -ci \
...
find "${ROOT_DIR}/apps" \
  ...
  xargs -0 "${ROOT_DIR}/tools/google-style/bin/ktlint" -F
