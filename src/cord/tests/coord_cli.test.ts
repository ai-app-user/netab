...
test("coord_cli parses the new sender-command-target grammar", async () => {
  ...
  const targeted = await parseInvocation(["@home", "-echo", "@vps", "hello", "--verbose"]);
  ...
  const payload = await parseInvocation(["@A", "-echo", "@B", "@./src/cord/playground/samples/bigfile.txt"]);
  assert.equal(payload.fullCmd, "foundation:echo");
  assert.equal(payload.payload?.kind, "bytes");
  assert.equal(payload.payload?.name, "./src/cord/playground/samples/bigfile.txt");
});
