...
test('cord foundation, cluster, election, and glue replication work together', async () => {
  ...
  assert.deepEqual(discovered.map((node) => node.nodeId).sort(), [
    'node-a',
    'node-b',
  ]);
  ...
  assert.equal(
    (
      await nodeA.call<{ nodeId: string }>(
        { nodeId: 'node-b' },
  ...
  assert.equal(
    (await nodeA.getLeader('offline', 'shard-b'))?.leaderNodeId,
    'node-b',
  );

  const orders = await leaderStore.ensure_table(
    'pos',
    'miami1',
    'orders',
    'json',
  ...
  const inserted = await leaderStore.add_obj(orders.tableId, {
    createdAt: Date.now(),
    status: 'PENDING',
  });
  ...
  const replicaTable = await replicaStore.get_table_info(
    'pos',
    'miami1',
    'orders',
  );
  assert.ok(replicaTable);
  const replicaRows = await replicaStore.get_objs(
...
test('cord IAM supports groups, credentials, and command grants', async () => {
  ...
  try {
    ...
    await node.createUser('tenant:abc', {
      userId: 'user:alice',
      displayName: 'Alice',
    });
    await node.setCredential('tenant:abc', 'user:alice', {
      type: 'password',
      secretHash: sha256Hex('secret-1'),
    });
    await node.addUserToGroup('tenant:abc', 'user:alice', 'staff');

    assert.equal(
      await node.verifyCredential('tenant:abc', 'user:alice', 'secret-1'),
      true,
    );
    assert.equal(
      await node.isMember('tenant:abc', 'ops', 'user:alice', {
        recursive: true,
      }),
      true,
    );
    ...
    await node.grant('tenant:abc', 'grp:staff', 'cmd:cord.cluster.exec', {
      allow: true,
      mask: 1,
    });

    assert.equal(
      await node.canInvoke(
        'tenant:abc',
        { userId: 'user:alice', groups: ['staff'] },
        'cmd:cord.cluster.exec',
        1,
      ),
      true,
    );
