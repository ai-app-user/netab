...
export async function startNetabHttpServer(service: NetabServiceLike, options: ServerOptions = {}) {
    ...
    try {
      ...
      switch (req.url) {
        case "/v1/netab/auth/anonymous":
          payload = await service.auth_anonymous(body as { brand?: string; siteId?: string });
          break;
        case "/v1/netab/auth/pin":
          payload = await service.auth_login_pin(String(body.pin));
          break;
        case "/v1/netab/access/redeem_join_code":
          payload = await service.access_redeem_join_code(String(body.joinCode));
          break;
        case "/v1/netab/access/request_location_access":
          payload = await service.access_request_location_access(String(body.locationId), body.proof ? String(body.proof) : undefined);
          break;
        case "/v1/netab/access/lookup_brand":
          payload = await service.access_lookup_brand(token, String(body.brandName));
          break;
        case "/v1/netab/access/list_sites_for_brand":
          payload = await service.access_list_sites_for_brand(token, String(body.brandName));
          break;
        ...
        case "/v1/netab/objs/get":
          payload = await service.get_objs(
            ...
            body.opts as { readMode?: "local_only" | "local_then_fallback" | "fallback_only" } | undefined,
          );
          break;
        case "/v1/netab/obj/add":
          payload = await service.add_obj(token, String(body.table_name), body.value);
          break;
        case "/v1/netab/objs/add":
          payload = await service.add_objs(token, String(body.table_name), body.rows as { value: unknown }[]);
          break;
        ...
        case "/v1/netab/objs/replace":
          payload = await service.replace_objs(token, String(body.table_name), body.ids as string[], body.values as unknown[]);
          break;
        case "/v1/netab/objs/delete":
          payload = await service.delete_objs(token, String(body.table_name), body.ids as string[]);
          break;
        default:
          res.writeHead(404).end();
          return;
