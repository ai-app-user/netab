...
import { assertPermission, assertWritableFields, filterReadableValue } from "./rbac.js";
import type { AccessResult, AclRule, JwtClaims, NetabContext, NetabGetResult, NetabServiceLike, PinRecord, ReadMode, SiteRecord, SubEvent, TablePolicy } from "./types.js";
...
function extractUpperBound(filter: Filter | null, field: string): number | undefined {
  if (!filter) {
    return undefined;
  }
  ...
  for (const [currentField, op, value] of filter) {
    ...
    if ((op === "<" || op === "<=") && typeof value === "number") {
      upperBound = upperBound === undefined ? value : Math.min(upperBound, value);
    }
    if (op === "between" && Array.isArray(value) && value.length === 2 && typeof value[1] === "number") {
      upperBound = upperBound === undefined ? value[1] : Math.min(upperBound, value[1]);
    }
  }
  return upperBound;
}

function inferIndexType(op: string, value: unknown): "str" | "i64" | "bool" | "time" {
  if (op === "between") {
    return "time";
  }
...
export class NetabDirectory {
  ...
  register(service: NetabService): void {
    const cluster = this.services.get(service.clusterId) ?? new Map<string, NetabService>();
...
export class NetabService implements NetabServiceLike {
  ...
  constructor(private readonly options: ServiceOptions) {
    ...
    this.siteScopedTables = new Set(options.siteScopedTables ?? ["menu_items", "orders", "payments", "events", "nodes"]);
  ...
  async auth_anonymous(args?: { brand?: string; siteId?: string }): Promise<AccessResult> {
  ...
  async access_request_location_access(locationId: string, proof?: string): Promise<AccessResult> {
    const site = Array.from(this.sites.values()).find((candidate) => candidate.locationId === locationId);
    if (!site) {
  ...
  async access_lookup_brand(token: string, brandName: string): Promise<SiteRecord[]> {
    this.requireAdmin(token);
    return this.findSitesByBrand(brandName);
  }

  async access_list_sites_for_brand(token: string, brandName: string): Promise<SiteRecord[]> {
    this.requireAdmin(token);
    return this.findSitesByBrand(brandName);
  }

  async access_resolve_domain(hostname: string): Promise<SiteRecord | null> {
    return Array.from(this.sites.values()).find((site) => site.hostname === hostname) ?? null;
  }
  ...
  async get_objs(token: string, table_name: string, ids: string[] | null = null, filter: Filter | null = null, start_pos = 0, max_count = -1, opts?: { readMode?: ReadMode }): Promise<NetabGetResult> {
    const ctx = this.requireContext(token);
    const action = filter && !ids ? "query" : "read";
    const permission = assertPermission(this.aclRules, ctx, table_name, action);
    const table = await this.getOrEnsureTable(table_name, this.policyFor(table_name).type ?? this.defaultTableType);
    const scopedFilter = this.applySiteScope(table_name, ctx, filter);
    await this.ensureIndexesForFilter(table, scopedFilter);
    const localResult = await this.steng.get_objs(table.tableId, ids, scopedFilter, start_pos, max_count);
    const filteredLocal = {
      ...localResult,
      items: localResult.items.map((item) => ({
        ...item,
        value: item.value !== undefined ? filterReadableValue(item.value, permission) : item.value,
      })),
    ...
    const fallback = await this.maybeFallbackRead(ctx, table_name, ids, scopedFilter, start_pos, max_count, filteredLocal, readMode);
    return fallback ?? filteredLocal;
  }

  async add_obj(token: string, table_name: string, value: unknown): Promise<{ id: string }> {
    const inserted = await this.add_objs(token, table_name, [{ value }]);
    return { id: inserted.ids[0] };
  }

  async add_objs(token: string, table_name: string, rows: { value: unknown }[]): Promise<{ ids: string[] }> {
    const ctx = this.requireContext(token);
    const permission = assertPermission(this.aclRules, ctx, table_name, "write");
    return this.routeWrite(table_name, async (leader) => {
      const table = await leader.getOrEnsureTable(table_name, leader.policyFor(table_name).type ?? leader.defaultTableType);
  ...
  async update_objs(token: string, table_name: string, ids: string[], patches: unknown[], opts?: { merge?: "deep" | "shallow" }): Promise<void> {
    const ctx = this.requireContext(token);
    const permission = assertPermission(this.aclRules, ctx, table_name, "write");
    await this.routeWrite(table_name, async (leader) => {
      const table = await leader.getOrEnsureTable(table_name, leader.policyFor(table_name).type ?? leader.defaultTableType);
      const rows = ids.map((id, index) => {
        const patch = leader.applyWriteScope(table_name, ctx, patches[index] ?? {});
  ...
  async replace_objs(token: string, table_name: string, ids: string[], values: unknown[]): Promise<void> {
    const ctx = this.requireContext(token);
    const permission = assertPermission(this.aclRules, ctx, table_name, "write");
    await this.routeWrite(table_name, async (leader) => {
      const table = await leader.getOrEnsureTable(table_name, leader.policyFor(table_name).type ?? leader.defaultTableType);
  ...
  async delete_objs(token: string, table_name: string, ids: string[]): Promise<void> {
    const ctx = this.requireContext(token);
  ...
  async subscribe_objs(token: string, table_name: string, filter: Filter | null, cb: (evt: SubEvent) => void): Promise<() => void> {
    const ctx = this.requireContext(token);
    const permission = assertPermission(this.aclRules, ctx, table_name, "subscribe");
    const table = await this.getOrEnsureTable(table_name, this.policyFor(table_name).type ?? this.defaultTableType);
    const scopedFilter = this.applySiteScope(table_name, ctx, filter);
    await this.ensureIndexesForFilter(table, scopedFilter);
    return this.steng.subscribe_objs(table.tableId, scopedFilter, (evt) => {
      cb({
        ...
        id: evt.id,
        ts: evt.ts,
        value: evt.value !== undefined ? filterReadableValue(evt.value, permission) : undefined,
      });
    });
  }

  private resolveAnonymousSite(args?: { brand?: string; siteId?: string }): SiteRecord {
    if (args?.siteId) {
    ...
    if (args?.brand) {
      const matches = this.findSitesByBrand(args.brand).filter((site) => site.public);
      if (matches.length !== 1) {
    ...
    const publicSites = Array.from(this.sites.values()).filter((site) => site.public);
  ...
  private async getOrEnsureTable(tableName: string, type: "json" | "binary"): Promise<TableInfo> {
    const existing = await this.steng.get_table_info(this.app, this.db, tableName);
    if (existing) {
      return existing;
    }
  ...
  private async ensureIndexesForFilter(table: TableInfo, filter: Filter | null): Promise<void> {
    if (!filter) {
      return;
    }
    for (const [field, op, value] of filter) {
      ...
      const idxType = inferIndexType(op, Array.isArray(value) ? value[0] : value);
  ...
  private applySiteScope(tableName: string, ctx: NetabContext, filter: Filter | null): Filter | null {
  ...
  private applyWriteScope(tableName: string, ctx: NetabContext, value: unknown): unknown {
  ...
  private async maybeFallbackRead(
    ...
    if (readMode === "fallback_only") {
      return this.readFromFallbacks(ctx, tableName, ids, filter, start_pos, max_count);
    }

    if (ids && localResult.items.some((item) => item.miss)) {
      const fallback = await this.readFromFallbacks(ctx, tableName, ids, filter, start_pos, max_count);
      if (!fallback) {
        return null;
      }
      const merged = fallback.items.map((fallbackItem) => {
        const localItem = localResult.items.find((item) => item.id === fallbackItem.id && !item.miss);
        return localItem ?? fallbackItem;
      });
      return { ...fallback, items: merged, source: "fallback" };
    }

    if (!ids && localResult.items.length === 0 && (this.policyFor(tableName).readFallbackClusters?.length ?? 0) > 0) {
      return this.readFromFallbacks(ctx, tableName, ids, filter, start_pos, max_count);
    }
    ...
    if (watermark !== undefined && upperBound !== undefined && upperBound <= watermark) {
      return this.readFromFallbacks(ctx, tableName, ids, filter, start_pos, max_count);
    }
  ...
  private async readFromFallbacks(
    ...
    for (const clusterId of this.policyFor(tableName).readFallbackClusters ?? []) {
      ...
      const result = await leader.get_objs(ctx.token, tableName, ids, filter, start_pos, max_count, { readMode: "local_only" });
  ...
  return [
    {
      ...
      tables: {
        ...
        orders: { allow: ["write", "read"], writeFields: ["items", "notes", "customer", "source", "createdAt", "siteId", "brandId", "locationId"] },
      },
    },
    {
      group: "staff",
      tables: {
        ...
        menu_items: { allow: ["read", "query", "write", "delete", "subscribe"] },
    ...
    },
    {
      group: "installer",
      tables: {
        "*": { allow: ["admin", "read", "query", "write", "delete", "subscribe", "blob_add", "blob_delete"] },
      },
    },
    {
      group: "admin",
