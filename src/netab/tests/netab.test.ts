...
test('netab routes writes to the primary cluster and enforces scoped customer permissions', async () => {
  ...
  const offlineA = await makeService({
    clusterId: 'offline',
    nodeId: 'offline-a',
    registry,
    directory,
  ...
  const offlineB = await makeService({
    clusterId: 'offline',
    nodeId: 'offline-b',
    registry,
    directory,
  ...
  const online = await makeService({
    clusterId: 'online',
    nodeId: 'online-a',
    registry,
    directory,
  ...
  await assert.rejects(() =>
    offlineA.add_objs(adminToken, 'menu_items', [
      {
        id: 'mi_taco',
        value: {
          itemId: 'mi_taco',
  ...
  const menuRead = await offlineB.get_objs(
    adminToken,
    'menu_items',
    null,
    [['brandId', '==', 'brand_brasao']],
  ...
  const anonymous = await offlineA.auth_anonymous({
    siteId: 'site_brasao_miami1',
  });
