...
async function main() {
  ...
  try {
    ...
    console.log("before retention", await steng.get_objs(table.tableId, inserted.ids, null, 0, 10));
    await steng.run_retention(now);
    console.log("after retention", await steng.get_objs(table.tableId, inserted.ids, null, 0, 10));
