...
test("postgres steng supports CRUD, query, retention, blobs, and remote op apply", async () => {
  ...
  const replicaTable = await replica.ensure_table("pos", "miami1", "orders", "json");
  ...
  const inserted = await primary.add_objs(table.tableId, [
    { value: { createdAt: now - 2 * 60 * 60 * 1000, equipmentId: "eq_2", status: "DONE" } },
    { value: { createdAt: now - 10 * 60 * 1000, equipmentId: "eq_1", status: "PENDING" } },
  ]);
  assert.match(inserted.ids[0], /^orders_pg1a_[0-9A-Z]{26}$/);
  assert.match(inserted.ids[1], /^orders_pg1a_[0-9A-Z]{26}$/);
  await primary.update_objs(table.tableId, [{ id: inserted.ids[1], patch: { nested: { value: 7 } } }]);
  const blob = await primary.add_blob(table.tableId, "blob_1", new Uint8Array([9, 8, 7]), "application/octet-stream");
  ...
  assert.deepEqual((queried.items[0].value as { nested: { value: number } }).nested, { value: 7 });
  ...
  const retained = await primary.get_objs(table.tableId, inserted.ids, null, 0, 10);
  assert.equal(retained.items[0].miss, "NOT_FOUND");
  assert.equal((await primary.get_blob(table.tableId, blob.id)).bytes.byteLength, 3);
  ...
  const replicaRows = await replica.get_objs(replicaTable.tableId, [inserted.ids[1]], null, 0, 10);
