...
test('sqlite steng persists tables, docs, blobs, and oplog across reopen', async () => {
  ...
  try {
    ...
    await first.update_objs(table.tableId, [
      { id: inserted.ids[0], patch: { nested: { value: 1 } } },
    ]);
    const blob = await first.add_blob(
      table.tableId,
      null,
    ...
    assert.equal(
      (await first.get_blob(table.tableId, blob.id)).bytes.byteLength,
      3,
    );
    await first.close();

    const reopened = new SqliteStengEngine({ filename, clusterShort: 'sq1a' });
    const reopenedTable = await reopened.get_table_info(
      'pos',
      'miami1',
      'orders',
    );
    ...
    assert.equal(
      (await reopened.read_ops_since(reopenedTable!.tableId, 0, 100)).length,
      3,
    );
    assert.equal(
      (await reopened.get_blob(reopenedTable!.tableId, blob.id)).contentType,
      'application/octet-stream',
    );
