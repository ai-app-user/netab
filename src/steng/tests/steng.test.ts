...
test("steng supports indexed CRUD, subscriptions, and retention", async () => {
  ...
  try {
    ...
    const events: string[] = [];
    const unsub = steng.subscribe_objs(table.tableId, [["equipmentId", "==", "eq_1"]], (evt) => {
    ...
    const added = await steng.add_objs(table.tableId, [
      {
        value: { createdAt: now - 30 * 60 * 1000, equipmentId: "eq_1", status: "PENDING" },
      },
      {
        value: { legacyOrderId: "legacy_ord_2", createdAt: now - 2 * 60 * 60 * 1000, equipmentId: "eq_2", status: "DONE" },
      },
    ]);
    assert.match(added.ids[0], /^orders_mi1a_[0-9A-Z]{26}$/);
    assert.match(added.ids[1], /^orders_mi1a_[0-9A-Z]{26}$/);

    await steng.update_objs(table.tableId, [{ id: added.ids[0], patch: { nested: { value: 1 } } }]);
    await steng.update_objs(table.tableId, [{ id: added.ids[0], patch: { nested: { other: 2 } } }]);
    ...
    assert.deepEqual((queried.items[0].value as { nested: { value: number; other: number } }).nested, {
      value: 1,
      other: 2,
    });
    assert.deepEqual(events, [`added:${added.ids[0]}`, `updated:${added.ids[0]}`, `updated:${added.ids[0]}`]);

    await steng.run_retention(now);
    const postRetention = await steng.get_objs(table.tableId, [added.ids[1]], null, 0, 10);
...
test("steng rejects caller-defined document ids on add_objs", async () => {
  ...
  try {
    ...
    await assert.rejects(() =>
      steng.add_objs(table.tableId, [{ id: "ord_manual", value: { status: "PENDING" } }] as unknown as { value: unknown }[]),
    );
  } finally {
    await steng.close();
  }
