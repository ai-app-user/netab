...
for (const backend of ["memory", "sqlite", "postgres"] as const satisfies readonly StengBackend[]) {
  test(`high-level Steng uses ${backend} with one backend setting`, async () => {
    ...
    try {
      ...
      assert.match(inserted.ids[0], new RegExp(`^orders_${backend.slice(0, 4)}_[0-9A-Z]{26}$`));

      const fetched = await steng.get_objs(table.tableId, inserted.ids, null, 0, 10);
      assert.equal(fetched.items.length, 1);
      assert.equal((fetched.items[0].value as { status: string }).status, "PENDING");
    } finally {
      await steng.close();
    }
  });
