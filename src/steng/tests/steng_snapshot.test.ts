...
for (const scenario of [
  ...
  test(`steng snapshot export/import works from ${scenario.sourceBackend} to ${scenario.targetBackend}`, async () => {
    ...
    const source = openSteng(
      scenario.sourceBackend,
      scenario.clusterShort,
      dir,
      `src-${scenario.sourceBackend}`,
    );
    const target = openSteng(
      scenario.targetBackend,
      `dst${scenario.clusterShort.slice(0, 1)}`,
      dir,
      `dst-${scenario.targetBackend}`,
    );

    try {
      const table = await source.ensure_table(
        'pos',
        'miami1',
        'orders',
        'json',
      ...
      await source.set_watermark(table.tableId, {
        localMinTimeMs: 1_709_999_000_000,
      });

      const outputPath =
        scenario.format === 'directory'
          ? join(dir, 'snapshot-dir')
          : join(dir, 'snapshot-export.tar.gz');
      ...
      const restoredTable = await target.get_table_info(
        'pos',
        'miami1',
        'orders',
      );
      ...
      const restoredRows = await target.get_objs(
        restoredTable!.tableId,
        [ready.id, cancelled.id],
        null,
        0,
        10,
      );
      assert.equal(
        (restoredRows.items[0].value as { status: string }).status,
        'READY',
      );
      assert.equal(restoredRows.items[1].miss, 'NOT_FOUND');
      assert.deepEqual(await target.get_watermark(restoredTable!.tableId), {
        localMinTimeMs: 1_709_999_000_000,
      });
      assert.equal(
        (await target.get_blob(restoredTable!.tableId, 'blob_receipt')).bytes
          .byteLength,
        4,
      );
      assert.equal(
        (await target.read_ops_since(restoredTable!.tableId, 0, 10)).length,
        3,
      );
    } finally {
      await source.close();
...
test('steng snapshot supports filtered exports and merge conflict handling', async () => {
  ...
  try {
    ...
    assert.equal(
      (restoredRows.items[0].value as { orderRef: string }).orderRef,
      'demo_200',
    );
    await assert.rejects(() =>
      target.get_blob(restoredTable!.tableId, 'blob_hidden'),
    );

    await target.update_objs(restoredTable!.tableId, [
      { id: ready.id, patch: { status: 'MODIFIED' }, merge: 'shallow' },
    ]);
    ...
    const afterSkip = await target.get_objs(
      restoredTable!.tableId,
      [ready.id],
      null,
      0,
    ...
    assert.equal(
      (afterSkip.items[0].value as { status: string }).status,
      'MODIFIED',
    );
    ...
    const afterReplace = await target.get_objs(
      restoredTable!.tableId,
      [ready.id],
      null,
      0,
    ...
    assert.equal(
      (afterReplace.items[0].value as { status: string }).status,
      'READY',
    );
  } finally {
    await source.close();
