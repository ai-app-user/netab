...
test('integration: leader writes replicate inside a cluster', async () => {
  ...
  const leaderCord = new CordNode(registry, {
    clusterId: 'offline',
    nodeId: 'leader',
    priority: 10,
    steng: leaderStore,
  });
  const replicaCord = new CordNode(registry, {
    clusterId: 'offline',
    nodeId: 'replica',
    priority: 5,
    steng: replicaStore,
  ...
  const replicaClient = createNetabClient({
    app: 'pos',
    db: 'miami1',
    service: replica,
  });
  ...
  assert.equal(
    (rows.items[0].value as { items: Array<{ sku: string }> }).items[0].sku,
    'taco',
  );
